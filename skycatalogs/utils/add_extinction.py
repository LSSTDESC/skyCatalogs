...
from skycatalogs.utils.creator_utils import make_MW_extinction_av, make_MW_extinction_rv
...
class AddExtinction():
    ...
    def write(self, pixel):
        infile = pq.ParquetFile(os.path.join(self._in_dir,
                                             f'{self._starts_with}{str(pixel)}.parquet'))
        ...
        writer = pq.ParquetWriter(os.path.join(self._out_dir,
                                               f'{self._starts_with}{str(pixel)}.parquet'),
                                  out_schema)
        ...
        for g in range(n_row_group):
            ...
            out_dict['MW_av'] = make_MW_extinction_av(out_dict['ra'], out_dict['dec'])
            out_dict['MW_rv'] = make_MW_extinction_rv(out_dict['ra'], out_dict['dec'])
